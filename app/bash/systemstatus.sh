...
# ResultString+=`curl $DomoticzURL'/json.htm?type='$Apichange'devices&rid=40' 2>/dev/null | jq -r '.result[].Data'`
# - or -
ResultString+=`curl $DomoticzURL'/json.htm?type='$Apichange'devices&rid=40' 2>/dev/null | jq -r '.result[].Temp | (. * 10 | round) / 10'`
ResultString+="°C\n"
